...
export default function FiberIntelligence({ fiber = {}, page = {} }) {
    const { props } = usePage();
    ...
    */
    ...
    const bySubsector = fiber?.by_subsector ?? [];

    const byFlow = fiber?.by_flow ?? [];

    const topImportProducts = fiber?.top_import_products ?? [];

    const topExportProducts = fiber?.top_export_products ?? [];

    const topImportOrigins = fiber?.top_import_origins ?? [];

    const topExportDestinations = fiber?.top_export_destinations ?? [];

    const importMarketShare = fiber?.import_market_share ?? [];

    const exportMarketShare = fiber?.export_market_share ?? [];

    const monthlyTrend = fiber?.monthly_trend ?? [];

    const yearlyTrend = fiber?.yearly_trend ?? [];

    const hs8Products = fiber?.hs8_products ?? [];

    const cottonOrigin = fiber?.cotton_origin_intelligence ?? {
        top_import_origins: [],
    ...
    */
    ...
    const labels = {
        title: isEn ? "Fiber Intelligence" : "Fiber Intelligence",

        subtitle: isEn
            ? "Fiber Trade Intelligence"
            : "Trade Intelligence Serat",
        ...
        cottonOrigin: isEn
            ? "Cotton Origin Intelligence"
            : "Intelligence Asal Kapas",

        cottonImportOrigins: isEn
            ? "Cotton Import Origins"
            : "Asal Impor Kapas",

        cottonExportDestinations: isEn
            ? "Cotton Export Destinations"
            : "Tujuan Ekspor Kapas",

        marketShare: isEn ? "Market Share" : "Pangsa Pasar",

        monthlyTrend: isEn ? "Monthly Trade Trend" : "Tren Perdagangan Bulanan",

        yearlyTrend: isEn ? "Yearly Trend" : "Tren Tahunan",
    ...
    */
    ...
    const cottonImportShare = useMemo(
        () => cottonOrigin?.import_market_share?.[0]?.market_share_percent ?? 0,
        [cottonOrigin],
    );

    const cottonExportShare = useMemo(
        () => cottonOrigin?.export_market_share?.[0]?.market_share_percent ?? 0,
        [cottonOrigin],
    );
    ...
    */
    ...
    const manMadeGroups = bySubsector.filter((item) =>
        ["synthetic", "artificial"].includes(item?.subsector),
    );

    const mainFiberGroups = bySubsector.filter(
        (item) => !["synthetic", "artificial"].includes(item?.subsector),
    );
                ...
                <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                    ...
                    <section className="mb-8">
                        <div className="mb-4">
                            ...
                            <p className="mt-1 text-sm text-slate-500">
                                {isEn
                                    ? `Current Fiber trade position through ${
                                          meta?.through_month ?? ""
                                      }/${meta?.current_year ?? ""}.`
                                    : `Posisi perdagangan Fiber sampai ${
                                          meta?.through_month ?? ""
                                      }/${meta?.current_year ?? ""}.`}
                            </p>
                        </div>
                        ...
                        <div className="mb-4 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-500">
                                    {isEn
                                        ? "Fiber market by product segment."
                                        : "Struktur pasar Fiber berdasarkan segmen produk."}
                                </p>
                            </div>
                        </div>
                            ...
                            {mainFiberGroups.map((item) => (
                                ...
                                >
                                    ...
                                    <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
                                        ...
                                        <div
                                            className={`flex items-center gap-1 text-xs font-black ${growthClass(
                        ...
                        {manMadeGroups.length > 0 && (
                            <div className="mt-4 rounded-2xl border border-indigo-100 bg-indigo-50/50 p-5">
                                <div className="mb-4">
                                    ...
                                    <p className="mt-1 text-sm text-slate-500">
                                        {isEn
                                            ? "Synthetic and artificial fiber segments."
                                            : "Segmen serat sintetis dan serat buatan."}
                                    </p>
                                </div>

                                <div className="grid gap-4 md:grid-cols-2">
                                    {manMadeGroups.map((item) => (
                                        <div
                                            key={item.subsector}
                                            className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"
                                        >
                                            ...
                                            <div
                                                className={`mt-4 flex items-center gap-1 text-xs font-black ${growthClass(
                                                    item.import_growth_percent,
                                                )}`}
                                            >
                                                {growthIcon(
                    ...
                    <section className="mb-8">
                        <div className="mb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-slate-500">
                                        {isEn
                                            ? `Cotton origin analysis through ${
                                                  meta?.current_year ?? 2026
                                              }.`
                                            : `Analisis asal kapas sampai ${
                                                  meta?.current_year ?? 2026
                                              }.`}
                                    </p>
                                </div>
                            ...
                            <div className="rounded-2xl border border-amber-100 bg-white p-6 shadow-sm">
                                <div className="mb-5">
                                    <div className="text-xs font-black uppercase tracking-[0.16em] text-amber-700">
                                        {labels.cottonImportOrigins}
                                    </div>

                                    <div className="mt-1 text-xs text-slate-400">
                                        {isEn
                                            ? "Based on actual country of origin in trade data."
                                            : "Berdasarkan negara asal aktual dalam data perdagangan."}
                                    </div>
                                </div>

                                <div className="space-y-4">
                                    {cottonOrigin?.top_import_origins
                                        ?.slice(0, 10)
                                        .map((item, index) => {
                                            const share =
                                ...
                                {cottonOrigin?.import_market_share?.length >
                                    0 && (
                                    <div className="mt-5 border-t border-slate-100 pt-4 text-xs text-slate-500">
                                        {labels.leadingOrigin}:{" "}
                                        <span className="font-black text-slate-700">
                                            {
                                                cottonOrigin
                                                    .import_market_share?.[0]
                                                    ?.country
                                            }
                                        </span>{" "}
                                        ({formatDecimal(cottonImportShare)}
                                        %)
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                                <div className="mb-5">
                                    <div className="text-xs font-black uppercase tracking-[0.16em] text-emerald-700">
                                        {labels.cottonExportDestinations}
                                    </div>

                                    <div className="mt-1 text-xs text-slate-400">
                                        {isEn
                                            ? "Cotton export destinations based on actual trade records."
                                            : "Tujuan ekspor kapas berdasarkan catatan perdagangan aktual."}
                                    </div>
                                </div>

                                <div className="space-y-4">
                                    {cottonOrigin?.top_export_destinations
                                        ?.slice(0, 10)
                                        .map((item, index) => {
                                            const share =
                                ...
                                {cottonOrigin?.export_market_share?.length >
                                    0 && (
                                    <div className="mt-5 border-t border-slate-100 pt-4 text-xs text-slate-500">
                                        {labels.leadingDestination}:{" "}
                                        <span className="font-black text-slate-700">
                                            {
                                                cottonOrigin
                                                    .export_market_share?.[0]
                                                    ?.country
                                            }
                                        </span>{" "}
                                        ({formatDecimal(cottonExportShare)}
                                        %)
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="mt-4 grid gap-4 md:grid-cols-3">
                            {["AMERIKA SERIKAT", "AUSTRALIA", "BRASILIA"].map(
                                (country) => {
                                    const item =
                                        cottonOrigin?.top_import_origins?.find(
                    ...
                    <section className="mb-8">
                        <div className="mb-4">
                            ...
                            <p className="mt-1 text-sm text-slate-500">
                                {isEn
                                    ? "Key movements from the current Fiber trade snapshot."
                                    : "Pergerakan utama dari snapshot perdagangan Fiber saat ini."}
                            </p>
                        </div>
                        ...
                        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
                            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                                ...
                                <div className="mt-1 text-xs text-slate-400">
                                    {isEn
                                        ? `Import market movement — ${meta?.current_year ?? 2026} vs ${meta?.comparison_year ?? 2025}`
                                        : `Pergerakan pasar impor — ${meta?.current_year ?? 2026} vs ${meta?.comparison_year ?? 2025}`}
                                </div>
                            </div>
                            ...
                            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                                ...
                                <div className="mt-1 text-xs text-slate-400">
                                    {isEn
                                        ? `Export market movement — ${meta?.current_year ?? 2026} vs ${meta?.comparison_year ?? 2025}`
                                        : `Pergerakan pasar ekspor — ${meta?.current_year ?? 2026} vs ${meta?.comparison_year ?? 2025}`}
                                </div>
                            </div>
                            ...
                            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                                ...
                                <div className="mt-1 text-xs text-slate-400">
                                    {isEn
                                        ? "Largest Fiber import origin"
                                        : "Negara asal impor Fiber terbesar"}
                                </div>
                            </div>
                            ...
                            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                                ...
                                <div className="mt-1 text-xs text-slate-400">
                                    {isEn
                                        ? "Largest Fiber export destination"
                                        : "Negara tujuan ekspor Fiber terbesar"}
                                </div>
                            </div>
                        </div>
                    </section>
                        ...
                        <div className="border-b border-slate-100 px-6 py-5">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-slate-500">
                                        {isEn
                                            ? "Detailed Fiber products by HS-8."
                                            : "Detail produk Fiber pada tingkat HS-8."}
                                    </p>
                                </div>
                            </div>
                        </div>
