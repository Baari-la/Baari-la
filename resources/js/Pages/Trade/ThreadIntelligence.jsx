...
export default function ThreadIntelligence({ thread = {}, page = {} }) {
    const { props } = usePage();
    ...
    */
    ...
    const bySubsector = thread?.by_subsector ?? [];

    const byFlow = thread?.by_flow ?? [];

    const topImportProducts = thread?.top_import_products ?? [];

    const topExportProducts = thread?.top_export_products ?? [];

    const topImportOrigins = thread?.top_import_origins ?? [];

    const topExportDestinations = thread?.top_export_destinations ?? [];

    const importMarketShare = thread?.import_market_share ?? [];

    const exportMarketShare = thread?.export_market_share ?? [];

    const monthlyTrend = thread?.monthly_trend ?? [];

    const yearlyTrend = thread?.yearly_trend ?? [];

    const hs8Products = thread?.hs8_products ?? [];
    ...
    */
    ...
    const labels = {
        title: isEn ? "Thread Intelligence" : "Thread Intelligence",
        ...
        currentPeriod: isEn ? "Current Period" : "Periode Berjalan",

        previousPeriod: isEn ? "Previous Period" : "Periode Sebelumnya",
    ...
    */
    ...
    const highestImportShare = useMemo(() => {
        return importMarketShare?.[0]?.market_share_percent ?? 0;
    }, [importMarketShare]);

    const highestExportShare = useMemo(() => {
        return exportMarketShare?.[0]?.market_share_percent ?? 0;
    }, [exportMarketShare]);
    ...
    */
    ...
    const maxTrendValue = useMemo(() => {
        const values = monthlyTrend.flatMap((item) => [
            Number(item?.import ?? 0),
            Number(item?.export ?? 0),
        ]);
                    ...
                    <section className="mb-8">
                        <div className="mb-4">
                            ...
                            <p className="mt-1 text-sm text-slate-500">
                                {isEn
                                    ? "Current trade position and year-on-year movement."
                                    : "Posisi perdagangan dan perubahan dibanding periode sebelumnya."}
                            </p>
                        </div>
                        ...
                        <div className="grid gap-5 md:grid-cols-2">
                            ...
                            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                                <div className="flex items-start justify-between">
                                    ...
                                    <div
                                        className={`flex items-center gap-1 ${growthClass(importGrowth)}`}
                                    >
                                ...
                                <div className="mt-5 grid grid-cols-2 gap-4 border-t border-slate-100 pt-4">
                                    ...
                                    <div>
                                        ...
                                        <div
                                            className={`mt-1 flex items-center gap-1 text-sm font-black ${growthClass(importGrowth)}`}
                                        >
                                            {importGrowth > 0 ? (
                            ...
                            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                                <div className="flex items-start justify-between">
                                    ...
                                    <div
                                        className={`flex items-center gap-1 ${growthClass(exportGrowth)}`}
                                    >
                                ...
                                <div className="mt-5 grid grid-cols-2 gap-4 border-t border-slate-100 pt-4">
                                    ...
                                    <div>
                                        ...
                                        <div
                                            className={`mt-1 flex items-center gap-1 text-sm font-black ${growthClass(exportGrowth)}`}
                                        >
                                            {exportGrowth > 0 ? (
                                                <TrendingUp className="h-4 w-4" />
                                            ) : exportGrowth < 0 ? (
                        ...
                        <div className="mb-4 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-500">
                                    {isEn
                                        ? "Thread market by product segment."
                                        : "Struktur pasar thread berdasarkan segmen produk."}
                                </p>
                            </div>
                        </div>
                            ...
                            {bySubsector.map((item) => (
                                ...
                                >
                                    ...
                                    <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
                                        ...
                                        <div
                                            className={`flex items-center gap-1 text-xs font-black ${growthClass(item.import_growth_percent)}`}
                                        >
                                            {growthIcon(
                            ...
                            <table className="min-w-[720px] w-full">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {(() => {
                                        ...
                                        /*
                    |--------------------------------------------------------------------------
                    | Display only through public reporting month
                    |--------------------------------------------------------------------------
                    */
...
===================================================== */}
                    ...
                    <section className="mb-8">
                        <div className="mb-4">
                            ...
                            <p className="mt-1 text-sm text-slate-500">
                                {isEn
                                    ? "Key movements from the current Thread trade snapshot."
                                    : "Pergerakan utama dari snapshot perdagangan Thread saat ini."}
                            </p>
                        </div>
                        ...
                        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
                            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                                ...
                                <div
                                    className={`mt-2 flex items-center gap-2 text-xl font-black ${growthClass(importGrowth)}`}
                                >
                                    {growthIcon(importGrowth)}
                                    {formatPercent(importGrowth)}
                                </div>
                            ...
                            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                                ...
                                <div
                                    className={`mt-2 flex items-center gap-2 text-xl font-black ${growthClass(exportGrowth)}`}
                                >
                                    {growthIcon(exportGrowth)}
                                    {formatPercent(exportGrowth)}
                                </div>
                            ...
                            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                                <div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">
                                    {isEn ? "Leading Origin" : "Asal Utama"}
                                </div>
                        ...
                        <div className="border-b border-slate-100 px-6 py-5">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-slate-500">
                                        {isEn
                                            ? "Detailed sewing thread products by HS-8."
                                            : "Detail produk thread pada tingkat HS-8."}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span>
                                {meta?.generated_at
                                    ? new Date(
                                          meta.generated_at,
                                      ).toLocaleString(isEn ? "en-US" : "id-ID")
                                    : "—"}
                            </span>
                        </div>
                    </div>
