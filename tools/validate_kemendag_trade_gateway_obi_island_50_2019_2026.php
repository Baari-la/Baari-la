...
echo "========================================\n";
echo "DIGESTEX KEMENDAG OBI ISLAND GATEWAY VALIDATION\n";
echo "========================================\n\n";

echo "SCOPE:\n";
echo "  YEARS       : 2019-2026\n";
echo "  TRADE FLOWS : EXPORT + IMPORT\n";
echo "  SOURCE      : OBI ISLAND\n";
echo "  SOURCE PROV : 20 - MALUKU UTARA\n";
echo "  CANDIDATE   : TERNATE PORT\n";
echo "  TRADE POINT : ID=50\n";
echo "  OPERATION   : READ ONLY\n\n";

$sourceName = 'OBI ISLAND';
$sourceProvinceId = 20;
$candidateId = 50;
...
$masterPass =
    ...
    &&
    (string) $master->name === 'Ternate Port'
    &&
...
echo "  CANDIDATE : " . ($masterPass ? 'PASS' : 'FAIL') . PHP_EOL;
echo "  ID        : " . ($master?->id ?? 'NULL') . PHP_EOL;
echo "  NAME      : " . ($master?->name ?? 'NULL') . PHP_EOL;
echo "  TYPE      : " . ($master?->trade_point_type_id ?? 'NULL') . PHP_EOL;
echo "  PROVINCE  : " . ($master?->province_id ?? 'NULL') . PHP_EOL;
echo "  CITY      : " . ($master?->city ?? 'NULL') . PHP_EOL;

if (!$masterPass) {
    throw new \RuntimeException(
        'OBI ISLAND candidate master validation failed.'
    );
}
...
if ($totalResidual !== 718) {
    throw new \RuntimeException(
        "Expected 718 OBI ISLAND residual records, got {$totalResidual}."
    );
}
...
$matchedCoverage =
    $totalResidual > 0
        ? ($matchedRecords / $totalResidual * 100)
        : 0;

$eligibleCoverage =
    $totalResidual > 0
        ? ($eligibleRecords / $totalResidual * 100)
        : 0;
...
echo "  SOURCE + CANDIDATE ID=50 : {$directCandidateResolved}\n";
echo "  EXPECTED                 : 0\n\n";
...
echo "========================================\n";
echo "OBI ISLAND TRADE GATEWAY VALIDATION : COMPLETE\n";
echo "========================================\n";